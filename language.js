...
const RU_UK = [
  ...
  ["🇷🇺 Русский", "🇷🇺 Русский"], ["🌐 Язык", "🌐 Мова"], ["🌐 Мова / Язык", "🌐 Мова / Язык"],
  ...
  ["Ваш ID:", "Ваш ID:"], ["❌ Telegram не авторизован.", "❌ Telegram не авторизований."],
  ["Telegram авторизован.", "Telegram авторизований."], ["Ошибка:", "Помилка:"],
  ["Источник добавлен.", "Джерело додано."], ["Приёмник добавлен.", "Приймач додано."],
...
function addLanguageButton(extra, l) {
  ...
  let keyboard = extra.reply_markup.keyboard.map(row => row.map(b => typeof b === "string" ? tr(b, l) : b));
  ...
  Composer.prototype.use = function(...middlewares) {
    if (!this.__postClonerLanguageMiddleware) {
      ...
      const languageMiddleware = async (ctx, next) => {
        ...
        if (text && lang(ctx.from?.id) === "uk") {
          ctx.message.text = tr(text, "ru");
        }
        return next();
      };
