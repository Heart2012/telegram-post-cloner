...
async function resolve(value){
  ...
  try{return await client.getEntity(v);}catch(e){throw new Error("Не удалось найти Telegram-чат. Проверь username/ссылку и доступ аккаунта.");}
}
...
async function processMessages(messages){
  ...
  for(const row of destFor.all(sourceChatId)){
    ...
    await enqueue(destinationChatId,async()=>{
      ...
      try{
        ...
        const sent=messages.length>1?await copyAlbum(messages,destination):await copyOne(messages[0],destination);
        if(!sent)return;
...
async function beginLogin(){
  ...
  c.start({
    ...
    console.error("Telegram authorization error:",e);
...
app.get("/health",(req,res)=>res.json({ok:true,telegram:!!client&&!loginInProgress,loginInProgress,telegramStarting,telegramError:telegramError||null,uptime:process.uptime()}));
...
app.post("/auth",async(req,res)=>{
  ...
  if(!value)return res.send(authPage("Пустое значение.","ERROR",AUTH_KEY,step));
  if(!["phone","code","password"].includes(step))return res.send(authPage("Неверный шаг.","ERROR",AUTH_KEY));
  if(!provideAuth(step,value))return res.send(authPage(`Telegram сейчас не ожидает шаг «${step}». Сначала введи предыдущий шаг.`,"ERROR",AUTH_KEY,step));
  const next=step==="phone"?"code":step==="code"?"password":"phone";
  res.send(authPage(step==="phone"?"Код отправлен в Telegram.":step==="code"?"Код передан. Если включена 2FA, введи пароль.":"Пароль передан. Жди завершения авторизации.","OK",AUTH_KEY,next));
});
