...
async function connectSavedSession(){
  ...
  try{
    ...
    await c.connect();
    if(!(await c.checkAuthorization())){await c.disconnect();clearSavedSession();telegramError="Сохранённая Telegram-сессия недействительна. Требуется повторная авторизация.";return;}
    const me=await c.getMe();client=c;saveSession(c.session.save());try{setupHandlers();}catch(e){console.error("Handler setup error after restore:",e);}console.log(`Telegram account restored: ${me.id} @${me.username||""}`);
  }catch(e){telegramError=e?.message||String(e);console.error("Session restore error:",e);if(c){try{await c.disconnect();}catch(_) {}}if(/AUTH_KEY_UNREGISTERED|SESSION_REVOKED|SESSION_EXPIRED/i.test(String(e?.message||e)))clearSavedSession();
  }finally{telegramStarting=false;}
}

async function beginLogin(){
  if(client)return true;if(loginInProgress)return true;loginInProgress=true;telegramError="";telegramStarting=true;
  const c=new TelegramClient(new StringSession(""),API_ID,API_HASH,{connectionRetries:10,autoReconnect:true});client=c;
  c.start({phoneNumber:async()=>waitAuth("phone"),password:async()=>waitAuth("password"),phoneCode:async()=>waitAuth("code"),onError:e=>console.error("Telegram auth:",e)})
  .then(async()=>{try{saveSession(c.session.save());const me=await c.getMe();try{setupHandlers();}catch(e){console.error("Handler setup error after login:",e);}console.log(`Telegram account authorized: ${me.id} @${me.username||""}`);telegramError="";}catch(e){telegramError=e?.message||String(e);console.error("Post-login initialization error:",e);if(client===c)client=null;try{await c.disconnect();}catch(_) {}}})
...
(async()=>{await bot.launch();console.log("Management bot started.");await connectSavedSession();process.once("SIGINT",()=>bot.stop("SIGINT"));process.once("SIGTERM",()=>bot.stop("SIGTERM"));})();
