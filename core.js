...
bot.use(async(ctx,next)=>{if(!ctx.from||isAdmin(ctx.from.id))return next();return ctx.reply("⛔ Доступ заборонено.\n\nВаш Telegram ID: "+ctx.from.id);});
...
bot.action("link_delete_menu",async ctx=>{await ctx.answerCbQuery();const ls=links();if(!ls.length)return ctx.reply("❌ Нет связок для удаления.",keyboard());return ctx.reply("🗑 Выбери связку:",Markup.inlineKeyboard(ls.map(r=>[Markup.button.callback(`${r.id}. ${r.source_title} → ${r.destination_title}`.slice(0,58),`link_del_${r.id}`)])));});
bot.action(/^link_del_(\d+)$/,async ctx=>{await ctx.answerCbQuery();const id=Number(ctx.match[1]);db.prepare("DELETE FROM links WHERE id=?").run(id);return ctx.reply("✅ Связка удалена.",keyboard());});
bot.action("link_cancel",async ctx=>{await ctx.answerCbQuery();state.delete(ctx.from.id);return ctx.reply("❌ Отменено.",keyboard());});
