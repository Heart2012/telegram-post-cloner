// Stable MTProto forwarder. Starts from TelegramClient.getMe(), which core.js
// calls after every successful connection/authorization. This avoids relying on
// update events or fragile connect/start monkey patches.
const fs = require("fs");
...
function setting(key, fallback=""){const r=settingStmt.get(key);return r?r.value:fallback;}
...
function sourceIdFromMessage(m){
  const p=m?.peerId||m?.peerID;
  if(p?.channelId!==undefined)return -1000000000000-idOf(p.channelId);
  if(p?.chatId!==undefined)return idOf(p.chatId);
  if(m?.chatId!==undefined)return idOf(m.chatId);
  if(m?.chatID!==undefined)return idOf(m.chatID);
  return 0;
}
...
function transformText(text){
  ...
  if(csv(setting("ban_words")).some(w=>lower.includes(w.toLowerCase())))return null;
  const keys=csv(setting("keywords"));if(keys.length&&!keys.some(w=>lower.includes(w.toLowerCase())))return null;
  if(setting("remove_links","0")==="1")text=text.replace(/https?:\/\/\S+|(?:https?:\/\/)?t\.me\/\S+/gi,"");
  for(const line of setting("replacements","").split(/\r?\n/)){if(!line.includes("->"))continue;const p=line.indexOf("->"),a=line.slice(0,p).trim(),b=line.slice(p+2).trim();if(a)text=text.split(a).join(b);}
  const sig=setting("signature","");if(sig)text=text.trim()?`${text.trim()}\n\n${sig}`:sig;
  return text.trim();
}
...
async function processOne(client,sourceId,m){
  const rows=destinationsFor.all(sourceId);
  console.log(`FORWARDER: source=${sourceId} message=${idOf(m.id)} destinations=${rows.length}`);
  for(const row of rows){const dest=idOf(row.chat_id);if(!dest)continue;if(copiedStmt.get(sourceId,idOf(m.id),dest))continue;await withLock(dest,async()=>{try{const entity=await client.getEntity(dest);const sent=await copyMessage(client,m,entity);if(!sent){console.log(`FORWARDER: filtered/empty ${sourceId}:${idOf(m.id)}`);return;}markStmt.run(sourceId,idOf(m.id),dest,idOf(sent.id));console.log(`FORWARDER COPIED ${sourceId}:${idOf(m.id)} -> ${dest}`);}catch(e){console.error(`FORWARDER COPY ERROR ${sourceId} -> ${dest}:`,e?.stack||e?.message||e);}});}
}
async function pollSource(client,row){
  const sourceId=idOf(row.chat_id);const ref=row.username?`@${row.username}`:sourceId;
  try{
    const entity=await client.getEntity(ref);const newest=await client.getMessages(entity,{limit:1});if(!newest?.length)return;
    const newestId=idOf(newest[0].id);const previous=lastSeen.get(sourceId);
    if(previous===undefined){lastSeen.set(sourceId,newestId);console.log(`FORWARDER: watching ${sourceId} (${row.title}), last=${newestId}`);return;}
    if(newestId<=previous)return;
    const messages=await client.getMessages(entity,{minId:previous,maxId:newestId,limit:Math.min(100,newestId-previous)});
    const fresh=(messages||[]).filter(m=>idOf(m.id)>previous&&idOf(m.id)<=newestId).sort((a,b)=>idOf(a.id)-idOf(b.id));
    lastSeen.set(sourceId,newestId);
    console.log(`FORWARDER: found ${fresh.length} new message(s) in ${sourceId}`);
    for(const m of fresh)await processOne(client,sourceId,m);
  }catch(e){console.error(`FORWARDER POLL ERROR ${sourceId}:`,e?.stack||e?.message||e);}
}
async function start(client){if(!client||running.has(client))return;running.add(client);console.log("FORWARDER: Telegram client ready; polling every 2 seconds.");const loop=async()=>{try{for(const row of sourceRows.all())await pollSource(client,row);}catch(e){console.error("FORWARDER LOOP ERROR:",e?.stack||e?.message||e);}setTimeout(loop,2000);};await loop();}

if(!TelegramClient.prototype.__postClonerGetMeHook){
  const originalGetMe=TelegramClient.prototype.getMe;
  TelegramClient.prototype.getMe=async function(...args){const result=await originalGetMe.apply(this,args);setImmediate(()=>start(this).catch(e=>console.error("FORWARDER START ERROR:",e?.stack||e?.message||e)));return result;};
  TelegramClient.prototype.__postClonerGetMeHook=true;
}
